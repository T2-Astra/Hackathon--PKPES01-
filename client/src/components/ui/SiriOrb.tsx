...
const SiriOrb: React.FC<SiriOrbProps> = ({
  ...
  // Use higher contrast for better color visibility at small sizes
  const finalContrast =
    sizeValue < 30
      ? 1.8 // Higher contrast for tiny sizes to show colors
      : sizeValue < 50
        ? Math.max(contrastAmount * 1.5, 1.6) // Increased contrast for small sizes
        : contrastAmount

  // Generate unique ID for this instance to avoid style conflicts
  const uniqueId = React.useMemo(() => `siri-orb-${Math.random().toString(36).substr(2, 9)}`, [])
    ...
    <>
      <style>
        {`
          ...
          .${uniqueId}[style*="--mask-radius: 0%"]::after {
            mask-image: none;
          }

          .${uniqueId}:not([style*="--mask-radius: 0%"])::after {
      ...
      <div
        className={cn(uniqueId, className)}
        style={
          {
            width: size,
            height: size,
